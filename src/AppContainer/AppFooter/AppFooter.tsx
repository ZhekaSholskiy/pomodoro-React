...
export function AppFooter() {
  const [isStatistic, setIsStatistic] = useState(!window.location.href.includes('statistic'));

  return (
    <div className={styles.container}>
      <div className={`${styles.sectionName} ${styles.logoName}`}>
        <img src={logo} alt="Лого приложения" className={styles.imgLogo}/>
        pomodoro_box
      </div>
      {isStatistic ?
      <Link to={'statistic'} onClick={() => setIsStatistic(false)}>
      <div className={`${styles.sectionName} ${styles.statName}`}>
        <svg className={styles.imgStat} width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M6 16H10V0H6V16ZM0 16H4V8H0V16ZM12 5V16H16V5H12Z" fill="#DC3E22"/>
        </svg>
        Статистика
      </div>
    </Link>:
    <Link to={'/'} onClick={() => setIsStatistic(true)}>
      <div className={`${styles.sectionName} ${styles.statName}`}>
        <svg className={styles.imgStat} width="20" height="20  " viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
          <g clip-path="url(#clip0_8795_152)">
